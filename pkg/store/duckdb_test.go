...
func TestMigrateOldSchema(t *testing.T) {
	// Create a store with the old schema (template_id + template columns).
	s, err := NewDuckDBStore("")
	if err != nil {
		t.Fatalf("NewDuckDBStore: %v", err)
	}
	...
	_, err = s.db.Exec(`CREATE SEQUENCE IF NOT EXISTS log_entries_id_seq START 1`)
	if err != nil {
	...
	_, err = s.db.Exec(
		`INSERT INTO log_entries (line_number, timestamp, raw, template_id, template) VALUES (?, ?, ?, ?, ?)`,
		1, time.Now(), "Starting server on port 8080", "D1", "Starting <*> on port <*>",
	)
	if err != nil {
		t.Fatalf("insert old row: %v", err)
	}

	// Run Init — should migrate the old schema.
	if err := s.Init(); err != nil {
		t.Fatalf("Init (migration): %v", err)
	}

	// Verify the migrated table has pattern_id and no template columns.
	var patternID string
	err = s.db.QueryRow(`SELECT pattern_id FROM log_entries WHERE line_number = 1`).Scan(&patternID)
	if err != nil {
		t.Fatalf("query migrated row: %v", err)
	}
	if patternID != "D1" {
		t.Errorf("pattern_id: got %q, want %q", patternID, "D1")
	}

	// Verify template column was dropped.
	var hasTemplate bool
	err = s.db.QueryRow(`
		SELECT COUNT(*) > 0 FROM information_schema.columns
		WHERE table_name = 'log_entries' AND column_name = 'template'
	`).Scan(&hasTemplate)
	if err != nil {
		t.Fatalf("check template column: %v", err)
	}
	if hasTemplate {
		t.Error("expected template column to be dropped after migration")
	}

	// Verify new inserts work with the migrated schema.
	entry := LogEntry{
		LineNumber: 2,
		Timestamp:  time.Now(),
		Raw:        "Connection timeout after 5000 ms",
		PatternID:  "D2",
	}
	if err := s.InsertLog(entry); err != nil {
		t.Fatalf("InsertLog after migration: %v", err)
	}
}
